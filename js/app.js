var app = new Vue({
	...
	methods: {
		bindRelationship(){
			...
			// return
			// http://127.0.0.1:8848/web3j-new/web3j/index.html
			var _this = this,account = web3.eth.coinbase,search = window.location.search;
			var urlParams = {}
			if(search){
		...
		getUserData(){
			var _this = this,account = web3.eth.coinbase;
			$.ajax({
				type: 'get',
		...
		insertTran(addr){
			...
			$.ajax({
				async:false,
				type: 'post',
				url: _this.baseUrl + "/user/invest",
				data: { 
					"tradingAddr": addr
				},
				success: function (data) {
					console.log("插入一条数据：" , data);
				}
			});
		},
		gameInvest(addr){
			...
			$.ajax({
				type: 'post',
				url: _this.baseUrl + "/user/gameInvest",
				data: { 
					"tradingAddr": addr
				},
				success: function (data) {
					console.log("游戏插入：" , data);
					_this.getTimeDown();
				}
			});
		},
		...
		getGwa: function(num) {
			...
			_this.contracts.GwaToken.deployed().then(function(instance) {
				...
				console.log(err.message + "at getGwa Error");
			});
		},
		...
		buyGwa: function(addr) 
		{
		   ...
		   $.ajax({
			async:false,
			type: 'post',
			url: _this.baseUrl + "/user/buyGwa",
			data: { 
			 "tradingAddr": addr
			},
			success: function (data) {
			 console.log("buyGwa：" , data);
			 _this.title = true;
			 setTimeout(function (){
				 _this.getGwaBalance();
			 }, 5000);
		...
		transferEth: function(value,type) {
			...
			if(type==4){
				...
				_this.contracts.GwaToken.deployed().then(function(instance) {
					...
					//_this.buyGwa(result.receipt.transactionHash);
				}).catch(function(err) {
					console.log(err.message + " at transferEth error");
				});
			}else{
				console.log(account)
				_this.contracts.MyToken.deployed().then(function(instance) {
					return instance.pay({
						value: value * 10 ** 18
					})
				}).then(function(result) {
					console.log(result);
					console.log(" at transferEth");
					_this.getEthBalance();
					if(type==2){
						//_this.insertTran(result.receipt.transactionHash);
					}else if(type==3){
						//结算
						_this.settlement(result.receipt.transactionHash);
					}
					// _this.bindRelationship();
				}).catch(function(err) {
					console.log(err.message + " at transferEth error");
				});
			}
		...
		getNumber: function() {
			var _this = this,
				account = web3.eth.coinbase;
			_this.contracts.MyToken.deployed().then(function(instance) {
				return instance.gwtNumber();
			}).then(function(result) {
				console.log(result + " at getNumber");
			}).catch(function(err) {
				console.log(err.message + " at getNumber Error");
			});
		...
		getBalance: function() {
			...
			// account 钱包地址
			_this.contracts.MyToken.deployed().then(function(instance) {
				// return instance.getBalance("0xeBA9640BE7F80AC5f7ae0eF7d9e48703eEd9816d");
				return instance.getBalance();
			}).then(function(result) {
				console.log(result + "合约钱包 at getBalance");
			}).catch(function(err) {
				console.log(err.message + "合约钱包 at getBalance Error");
			});
		...
		getGwaBalance: function() {
			...
			_this.contracts.GwaToken.deployed().then(function(instance) {
				...
				console.log(result + "Gwa合约钱包 at getGwaBalance");
				_this.gwaBalance = result / 10 ** 18;
				_this.gwaToEthBalance = result / 10 ** 18 / _this.gwaToEth;
			}).catch(function(err) {
				console.log(err.message + "合约钱包 at getGwaBalance Error");
			});
		},
...
//				var time = parseInt(result[1]) + (n + 1) * parseInt(result[2]) - parseInt(new Date().getTime() / 1000);
	...
	mounted: function() {
		...
		// let date1 = new Date('2019-05-08').getTime();
		// let date2 = new Date('2019-05-09').getTime();
